...
class XPTableView: UIScrollView {
    ...
    private func generateHeightAndOffsetData() {
        var checkHeightForEachRow = false

        if let respondsToHeightMethod = xpDelegate?.responds(to: #selector(XPTableViewDelegate.xpTableView(heightForRow:))) {
            checkHeightForEachRow = respondsToHeightMethod
        }

        var currentOffsetY: CGFloat = 0
        var newRowRecords = Array<RowRecord>()

        let numberOfRows = xpDataSource!.numberOfRows(in: self)
        ...
        contentSize = CGSize(width: bounds.width, height: currentOffsetY)
    }
    ...
    private func findRow(for offsetY: CGFloat, inRange: NSRange) -> Int {
        ...
        let index = (rowRecords as NSArray).index(of: record, inSortedRange: inRange, options: NSBinarySearchingOptions.insertionIndex) { (l: Any, r: Any) -> ComparisonResult in
